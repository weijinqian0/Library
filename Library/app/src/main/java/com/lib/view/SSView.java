...
public class SSView extends View {
	...
	/** 是否显示缩略图 */
	private boolean showThumView = true;
...
//		this.iMaxPay = imaxPay;
	...
	protected void onDraw(Canvas paramCanvas) {
		...
		//获取座位的状态信息，加载座位布局   i3,seatNum i2,rowNum
		for (int i2 = 0; i2 < this.mListSeatConditions.size(); i2++) {
			...
			if(this.mListSeatInfos != null && this.mListSeatInfos.size()>0){
				for (int i3 = 0; i3 < this.mListSeatInfos.get(i2).getSeatList()//i2行的SeatList
						.size(); i3++) {// 2344
					// goto5 - 2344
					Seat localSeat = this.mListSeatInfos.get(i2).getSeat(i3);//i2行 i3列的座位信息
					switch (((Integer) localArrayList.get(i3)).intValue()) { // 往三种状态不同的座位加载不同的图片信息
						case 0: // 2401 - 走道
		...
		for (int i1 = 0; i1 < this.mListSeatInfos.size(); i1++) {
			...
			paramCanvas.drawRect(new Rect((int) Math.abs(this.n),
					this.j + i1 * this.ss_seat_current_height,
	...
	/**
	 * 是否显示缩略图 param为true时显示
	 *
	 * @param mSsView
	 * @param param
	 * @return
	 */
	public static boolean showThum(SSView mSsView, boolean param) {
		mSsView.showThumView = param;
		return mSsView.showThumView;
	}
	 ...
	 * @return
	 */
	public static OnSeatClickListener getListener(SSView mSsView) {
	 ...
	 * @return
	 */
	public static int getColumn(SSView mSsView, int param) {
		return mSsView.getColumn(param);
	}
	 ...
	 * @return
	 */
	private int getColumn(int paramX) {
		try {
			int i1 = (paramX + this.p - this.h) / this.ss_seat_current_width;
			//i2 = i1 - i1 / 4 - 1;
			return i1;
		} catch (Exception localException) {
			localException.printStackTrace();
	 ...
	 * @return
	 */
	public static int getRow(SSView mSsView, int param) {
		return mSsView.getRow(param);
	}
	 ...
	 * @return
	 */
	private int getRow(int paramY) {
		try {
			int i1 = (paramY + this.q - this.j) / this.ss_seat_current_height;//i为点击位置的真实的排数
			//int i2 = i1 - (i1 / 3);//计算有几个走道
			return i1;
